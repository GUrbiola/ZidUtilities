...
namespace ZidUtilities.CommonCode.Win.Controls.Grid.GridFilter.GridFilterFactories
{
	...
	public class DefaultGridFilterFactory : GridFilterFactoryBase
	{
		...
		public DefaultGridFilterFactory()
		{
			...
			// Binary data types should not be filterable
			AddGridFilter(typeof(byte[]), typeof(EmptyGridFilter));

			// Handle SQL Server specific binary types
			try
			{
				Type sqlBinaryType = Type.GetType("System.Data.SqlTypes.SqlBinary, System.Data");
				if (sqlBinaryType != null)
					AddGridFilter(sqlBinaryType, typeof(EmptyGridFilter));
			}
			catch { }

			try
			{
				Type linqBinaryType = Type.GetType("System.Data.Linq.Binary, System.Data.Linq");
				if (linqBinaryType != null)
					AddGridFilter(linqBinaryType, typeof(EmptyGridFilter));
			}
			catch { }
		}
